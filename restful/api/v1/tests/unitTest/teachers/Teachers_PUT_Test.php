...
//base url: /teachers
class Teachers_PUT_Test extends BasicTestCase
{

    const TEACHER_ID = 3342;
    protected $serviceDataArray = array(
        'method' => 'PUT',
        'url' => [Teachers_PUT_Test::TEACHER_ID]); // url: /teachers/{teacherId}
    protected $requiredInputsArray = ['accessToken', 'username', 'firstName', 'lastName', 'isAdmin'];
    protected $basenameFile = 'Teachers.php';

    public function testTeachersPUT_SuccessConsumerNotTeacher()
    {
        //Defining the mocked methods
        $this->createUtilMock(['checkInput', 'nullCheckForTables']);
        $this->createTableMock('TeacherTbl', ['isTeacherAdmin', 'updateTeacherAccount']);
        ...
        $teacherId = Teachers_PUT_Test::TEACHER_ID;
        $username = 'teacher1';
        $firstName = 'Teacher1';
        $lastName = 'LastName1';
        $isAdmin = 'true';
        ...
        $details = array('teacherId' => $teacherId);

        $jsonInput = $this->createInputJson($accessToken, $username, $firstName, $lastName, $isAdmin);
        ...
        $this->_teacherTblStub->expects($this->once())
            ->method('updateTeacherAccount')
            ->with(
                $this->stringContains($consumerId)
                , $this->stringContains($teacherId)
    ...
    public function testTeachersPUT_SuccessConsumerIsTeacher()
    {
        //Defining the mocked methods
        $this->createUtilMock(['checkInput', 'nullCheckForTables']);
        $this->createTableMock('TeacherTbl', ['isTeacherAdmin', 'updateTeacherAccount']);
    ...
    public function testTeachersPUT_EmptyInput_All()
    {
        //Defining the mocked methods
    ...
    public function testTeacherPUT_InvalidToken()
    {
        //Defining the mocked methods
        $this->createUtilMock(['checkInput', 'nullCheckForTables']);
        $this->createTableMock('TokenTbl', ['retrieveToken']);
    ...
    public function testTeacherPUT_ExpiredToken()
    {
        //Defining the mocked methods
        $this->createUtilMock(['checkInput', 'nullCheckForTables']);
        $this->createTableMock('TokenTbl', ['retrieveToken']);
    ...
    public function testTeacherPUT_CatchDatabaseException()
    {
        //Defining the mocked methods
        $this->createUtilMock(['checkInput', 'nullCheckForTables']);
        $this->createTableMock('TeacherTbl', ['isTeacherAdmin', 'updateTeacherAccount']);
        ...
        $details = "$pdoMessage [FILE: C:\\wamp64\\www\\php\\restful\\api\\v1\\tests\\unitTest\\teachers\\Teachers_PUT_Test.php] [LINE: 384]";
        $httpCode = 500;
    ...
    public function testTeacherPUT_DatabaseDependencyNull()
    {
    ...
    private function createInputJson($accessToken, $username, $firstName, $lastName, $isAdmin)
    {
        return array(
